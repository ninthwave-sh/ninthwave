...
1. **No persistent/watch mode:** The daemon exits when all items reach terminal state. There's no mode where it keeps running and watches for new TODOs, new PRs to review, or manually-retried items.
...
**Suggestion:**
- Add `--watch` or `--persistent` mode: daemon stays running, polls for new TODO files, watches for external PRs to review
