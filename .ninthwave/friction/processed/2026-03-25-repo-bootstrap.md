# Friction: Cross-repo TODO can't bootstrap a new repo
...
1. **Chicken-and-egg bootstrapping:** When a TODO's job is to *create* a repo (scaffold), the orchestrator can't launch a worker because it needs the repo directory to exist first. The worker launch fails with "Repo 'policy-proxy' not found" and the item goes to `stuck`.
...
- **Bootstrap support:** When a TODO has `Repo: X` and repo X doesn't exist, the orchestrator could create an empty GitHub repo + local clone before launching the worker. Or flag it as a "bootstrap TODO" that runs in the hub repo with a mandate to create the target repo.
